...
- **Architecture**: Do not introduce new top-level feature roots in the repository root. Prefer feature-local co-location first: keep UI and closely related helper/state logic together under `components/<feature>/...` when that makes the feature easier to reason about, and hoist only genuinely shared or cross-feature logic into `lib/<feature>/...`. Avoid ad hoc root folders like `openroutermcp/`.
- **Multilingual/i18n**: Do not handle localized UI text or language-specific logic with two-way ternary expressions such as `locale === "zh" ? ... : ...`. Use an extensible locale map, message table, or i18n helper keyed by language so future languages can be added without rewriting condition chains.
