...
function showcaseIssueTitle(project: Project | null): string {
  return `Showcase: ${project?.title.trim() || "project"}`;
}

const showcaseIssueBodyCopy = {
  zh: {
    authorPlaceholder: "<!-- 请输入作者名、团队名或 GitHub 用户名 -->",
    thumbnailPlaceholder: "<!-- 请拖入一张代表性缩略图，或粘贴图片 URL -->",
    introPlaceholder: "<!-- 请用 1-3 句话介绍作品内容、亮点或适合展示的原因 -->",
    attachmentIntro:
      "<!-- 请上传从 GenStory.cc 导出的源码 ZIP、可运行 ZIP、OpenWebGal ZIP、Phaser ZIP 或 PDF。 -->",
    generatedNote:
      "由 GenStory.cc 自动生成提交模板；提交前请补全作者、缩略图、简介和附件。",
  },
  en: {
    authorPlaceholder: "<!-- Enter the author name, team name, or GitHub username -->",
    thumbnailPlaceholder: "<!-- Drag in a representative thumbnail or paste an image URL -->",
    introPlaceholder: "<!-- Introduce the work in 1-3 sentences: story, highlights, or why it belongs in the showcase -->",
    attachmentIntro:
      "<!-- Upload the source ZIP, playable ZIP, OpenWebGal ZIP, Phaser ZIP, or PDF exported from GenStory.cc. -->",
    generatedNote:
      "Generated by GenStory.cc. Please complete the author, thumbnail, introduction, and attachment fields before submitting.",
  },
} satisfies Record<"zh" | "en", Record<string, string>>;

function showcaseIssueBody(project: Project | null): string {
  const issueLang = project?.lang ?? "zh";
  const copy = showcaseIssueBodyCopy[issueLang];
  const title = project?.title.trim() || "Untitled";
  const type = project ? contentTypeById[project.template]?.label[issueLang] ?? project.template : "";
...
function showcaseIssueUrl(project: Project | null): string {
  const params = new URLSearchParams({
    ...
    title: showcaseIssueTitle(project),
    body: showcaseIssueBody(project),
  });
...
export default function EditorClient() {
  ...
  async function handlePreview() {
    ...
    router.push(localizedPath(lang, `projects/preview?id=${project.id}`));
  }
  ...
  const showcaseBody = showcaseIssueBody(project);
    ...
    <main className="flex h-svh flex-col overflow-hidden">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <Button render={<Link href={localizedPath(lang, "projects")} />} variant="ghost" size="icon" aria-label={t("editor.back")}>
      ...
      >
        <div className="rounded-lg border bg-muted/40 p-3 text-sm">
          ...
          <p className="mt-3 font-medium">{t("editor.showcaseIssueTemplateLabel")}</p>
          <pre className="mt-1 max-h-44 overflow-auto whitespace-pre-wrap rounded-md bg-background p-3 text-xs text-muted-foreground">
            {showcaseBody}
          </pre>
          <Button
