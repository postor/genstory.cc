...
test("chatbox enables the default goal contract and persists its state", async () => {
  ...
  assert.match(source, /goal\.status === "blocked" \|\|[\s\S]*goal\.status === "stalled" \|\|[\s\S]*goal\.status === "active" && Boolean\(goal\.blocker\)/);
  assert.match(source, /goalForTurn = \{[\s\S]*\.\.\.goalForTurn,[\s\S]*blocker: message,[\s\S]*updatedAt: Date\.now\(\),[\s\S]*\}/);
  ...
  assert.doesNotMatch(source, /onMouseEnter=\{\(\) => setGoalDetailsOpen\(true\)\}/);
  assert.doesNotMatch(source, /onMouseLeave=\{\(\) => setGoalDetailsOpen\(false\)\}/);
  assert.doesNotMatch(source, /onFocus=\{\(\) => setGoalDetailsOpen\(true\)\}/);
  assert.doesNotMatch(source, /onBlur=\{\(\) => setGoalDetailsOpen\(false\)\}/);
  assert.doesNotMatch(source, /onMouseEnter=\{\(\) => setGoalObjectiveOpen\(true\)\}/);
  assert.doesNotMatch(source, /onMouseLeave=\{\(\) => setGoalObjectiveOpen\(false\)\}/);
  assert.doesNotMatch(source, /onFocus=\{\(\) => setGoalObjectiveOpen\(true\)\}/);
  assert.doesNotMatch(source, /onBlur=\{\(\) => setGoalObjectiveOpen\(false\)\}/);
