...
**Goal:** Let a user back up and restore the global GenStory workspace with Google Drive or Dropbox, without a GenStory account, backend, or ZIP-based cloud transfer.
...
- Files are stored under an isolated provider app folder. Google Drive and Dropbox use `GenStory Workspace - Local Story Projects`.
