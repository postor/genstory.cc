...
const zh: Dict = {
  ...
  "meta.settingsTitle": "设置 - GenStory",

  "settings.title": "设置",
  "settings.cloud.subtitle": "把浏览器中的作品同步到你自己的云盘。",
  "settings.cloud.title": "云端同步",
  "settings.cloud.description": "选择一个云盘并授权。GenStory 只访问自己的应用目录，不会读取或修改目录外的文件。",
  ...
  "settings.cloud.rememberHint": "关闭后令牌只保存在当前浏览器会话中。令牌不会上传到 GenStory。",
  "settings.cloud.savePreferences": "保存同步偏好",
  "settings.cloud.preferencesSaved": "同步偏好已保存",
  "settings.cloud.folderNote": "Google Drive 会创建“GenStory Workspace - Local Story Projects”文件夹；OneDrive 使用应用专属 approot；Dropbox 使用应用授权根下的同名文件夹。同步只新增或覆盖对应相对路径的文件，不删除其他文件。",
  "settings.cloud.guideButton": "连接说明",
  "settings.cloud.guideTitle": "连接云盘",
  "settings.cloud.guideDescription": "OAuth 授权直接在云盘官方页面完成，GenStory 没有账号系统或后端代理。",
  "settings.cloud.guideSteps": "Google Drive 已内置公开 client ID；OneDrive 和 Dropbox 需要部署者提供公开 client ID，并把 /settings 加入允许的回调地址。连接后，项目页会按项目类型、项目 ID 和文件相对路径同步到云盘目录。",
  "settings.cloud.apiDocs": "API 文档",
  "settings.cloud.oauthDocs": "OAuth 文档",
  "settings.cloud.clientIdNote": "client ID 是公开标识，不是 client secret。Google Drive 的 client ID 已内置；不要把 client secret、服务账号密钥或 refresh token 粘贴到页面。OneDrive/Dropbox 的公开 client ID 通过 NEXT_PUBLIC_ONEDRIVE_CLIENT_ID、NEXT_PUBLIC_ONEDRIVE_TENANT_ID、NEXT_PUBLIC_DROPBOX_APP_KEY 配置。",
...
const en: Dict = {
  ...
  "meta.settingsTitle": "Settings - GenStory",

  "settings.title": "Settings",
  "settings.cloud.subtitle": "Sync works from this browser to a cloud drive you control.",
  "settings.cloud.title": "Cloud sync",
  "settings.cloud.description": "Choose a drive and authorize it. GenStory only uses its own app directory and does not read or modify files outside it.",
  ...
  "settings.cloud.rememberHint": "When off, the token stays in this browser session. Tokens are never sent to GenStory.",
  "settings.cloud.savePreferences": "Save sync preferences",
  "settings.cloud.preferencesSaved": "Sync preferences saved",
  "settings.cloud.folderNote": "Google Drive creates “GenStory Workspace - Local Story Projects”; OneDrive uses its app-only approot; Dropbox uses the same named folder under the app authorization root. Sync only adds or replaces matching relative paths and never deletes other files.",
  "settings.cloud.guideButton": "Connection guide",
  "settings.cloud.guideTitle": "Connect a cloud drive",
  "settings.cloud.guideDescription": "Authorization happens on the provider's official page. GenStory has no account system or backend proxy.",
  "settings.cloud.guideSteps": "Google Drive includes a built-in public client ID. OneDrive and Dropbox need public client IDs supplied by the deployment, plus /settings as an allowed redirect URI. After connecting, Projects syncs by project type, project ID, and relative file path.",
  "settings.cloud.apiDocs": "API docs",
  "settings.cloud.oauthDocs": "OAuth docs",
  "settings.cloud.clientIdNote": "A client ID is public, not a client secret. Google Drive's client ID is built in. Never paste a client secret, service-account key, or refresh token into the page. Configure OneDrive/Dropbox with NEXT_PUBLIC_ONEDRIVE_CLIENT_ID, NEXT_PUBLIC_ONEDRIVE_TENANT_ID, and NEXT_PUBLIC_DROPBOX_APP_KEY.",
