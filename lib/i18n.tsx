...
const zh: Dict = {
  ...
  "settings.cloud.folderNote": "Google Drive 会创建“GenStory Workspace - Local Story Projects”文件夹；Dropbox 使用应用授权根下的同名文件夹。同步只新增或覆盖对应相对路径的文件，不删除其他文件。",
  ...
  "settings.cloud.guideSteps": "Google Drive 和 Dropbox 都使用内置公开 client ID；请在对应平台把 /settings 加入允许的回调地址。连接后，项目页会按项目类型、项目 ID 和文件相对路径同步到云盘目录。",
  "settings.cloud.apiDocs": "API 文档",
  "settings.cloud.oauthDocs": "OAuth 文档",
  "settings.cloud.clientIdNote": "client ID 是公开标识，不是 client secret。Google Drive 和 Dropbox 的公开 client ID 已内置；不要把 client secret、服务账号密钥或 refresh token 粘贴到页面。",
...
const en: Dict = {
  ...
  "settings.cloud.folderNote": "Google Drive creates “GenStory Workspace - Local Story Projects”; Dropbox uses the same named folder under the app authorization root. Sync only adds or replaces matching relative paths and never deletes other files.",
  ...
  "settings.cloud.guideSteps": "Google Drive and Dropbox use built-in public client IDs. Add /settings as an allowed redirect URI on the provider platform. After connecting, Projects syncs by project type, project ID, and relative file path.",
  "settings.cloud.apiDocs": "API docs",
  "settings.cloud.oauthDocs": "OAuth docs",
  "settings.cloud.clientIdNote": "A client ID is public, not a client secret. Google Drive and Dropbox client IDs are built in. Never paste a client secret, service-account key, or refresh token into the page.",
