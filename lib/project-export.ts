...
function pictureBookRuntimeHtml(model: ProjectPreviewModel, lang: "zh" | "en"): string {
  const pages = JSON.stringify(model.sections.map((section) => ({
    ...
    position: section.textPosition ?? "bottom-left",
    size: section.textSize ?? "30px",
  }))).replace(/</g, "\\u003c");
  ...
  return `<!doctype html><html lang="${lang === "zh" ? "zh-CN" : "en"}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(model.title)}</title><style>:root{font-family:ui-serif,"Noto Serif SC",serif;color:#30271f;background:#151311}*{box-sizing:border-box}body{margin:0}main{min-height:100vh;display:grid;place-items:center;padding:24px}.reader{width:min(1120px,100%);color:#fff}.top{display:flex;justify-content:space-between;margin-bottom:12px;opacity:.75}.page{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(240px,.8fr);background:#fbf6e9;color:#30271f}.art{aspect-ratio:16/9;background:#fff}.art img{width:100%;height:100%;object-fit:cover}.copy{padding:32px;display:flex;flex-direction:column;justify-content:space-between;gap:24px}.copy h2{font-size:1rem;color:#876b52}.copy p{font-size:1.15rem;line-height:1.8}.voice-toggle{display:inline-flex;width:28px;height:28px;margin-left:8px;vertical-align:-6px;align-items:center;justify-content:center;border-radius:999px;border:1px solid rgba(59,36,24,.25);background:rgba(59,36,24,.10);color:rgba(59,36,24,.68);cursor:pointer}.voice-toggle:hover{background:rgba(59,36,24,.15);color:#3b2418}.voice-toggle svg{width:16px;height:16px;fill:currentColor;opacity:.75}.controls{display:flex;justify-content:center;align-items:center;gap:12px;margin-top:16px}.controls button{border:1px solid #ffffff55;background:transparent;color:#fff;padding:8px 14px;cursor:pointer}.controls button:disabled{opacity:.35;cursor:default}@media(max-width:700px){.page{grid-template-columns:1fr}.copy{padding:24px}.copy p{font-size:1rem}}</style></head><body><main><div class="reader"><div class="top"><span>${escapeHtml(model.title)}</span><span id="count"></span></div><section class="page"><div class="art"><img id="image" alt=""></div><div class="copy"><div><h2 id="title"></h2><p><span id="body"></span><button id="voiceButton" class="voice-toggle" type="button" aria-label="${labels.play}"><svg id="playIcon" viewBox="0 0 24 24"><path d="M9 7.4v9.2c0 .7.8 1.1 1.4.7l6.9-4.6c.5-.3.5-1.1 0-1.4l-6.9-4.6c-.6-.4-1.4 0-1.4.7Z"/></svg><svg id="stopIcon" viewBox="0 0 24 24" hidden><rect x="7" y="7" width="10" height="10" rx="2"/></svg></button></p></div><audio id="voice"></audio></div></section><div class="controls"><button id="prev">${labels.previous}</button><button id="next">${labels.next}</button></div></div></main><script>const pages=${pages};const labels=${JSON.stringify(labels)};let i=0;let playing=false;const $=id=>document.getElementById(id);function setPlaying(next){playing=next;$('playIcon').hidden=next;$('stopIcon').hidden=!next;$('voiceButton').setAttribute('aria-label',next?labels.pause:labels.play)}function render(){const p=pages[i];setPlaying(false);$('count').textContent=(i+1)+' / '+pages.length;$('title').textContent=p.title;$('body').textContent=p.body;$('image').src=p.image;$('image').alt=p.title;$('voice').src=p.voice;$('voiceButton').hidden=!p.voice;$('prev').disabled=i===0;$('next').disabled=i===pages.length-1;}$('prev').onclick=()=>{if(i>0){$('voice').pause();i--;render()}};$('next').onclick=()=>{if(i<pages.length-1){$('voice').pause();i++;render()}};$('voiceButton').onclick=()=>{const a=$('voice');if(!a.src)return;if(playing){a.pause();setPlaying(false)}else{a.play().then(()=>setPlaying(true)).catch(()=>setPlaying(false))}};$('voice').onended=()=>setPlaying(false);render();</script></body></html>`;
}
...
export async function exportPictureBookZip(root: FileSystemDirectoryHandle, model: ProjectPreviewModel, title: string, lang: "zh" | "en"): Promise<void> {
  ...
  for (const entry of files) {
    if (entry.kind !== "file" || (!entry.path.startsWith("chapter-") && entry.path !== "meta.md")) continue;
